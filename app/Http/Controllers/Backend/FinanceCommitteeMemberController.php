...
class FinanceCommitteeMemberController extends Controller
{
    public function index()
    {
        ...
        foreach ($committes as $key => $comitte) {
             if($comitte->committe_type_id == 4){
               array_push($comitteData,$comitte);
             }
        }
        // return $comitteData;
        return view('backend.regulatory-bodies.finance-committee-member.view',['comitteData'=>$comitteData]);
       
    }
    public function create()
    {
        $d = RegulatoryCommitte::where('committe_type_id',4)->pluck('profile_id');
        $data['profiles'] = Profile::whereNotIn('id',$d)->get(); 
        return view('backend.regulatory-bodies.finance-committee-member.add',$data);
    }
    public function store(Request $request)
    {
        ...
        return redirect()->route('finance.committee.member')->with('success','Data Saved successfully');
    ...
    {
        $d = RegulatoryCommitte::where('committe_type_id',4)->pluck('profile_id')->toArray();
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('finance.committee.member')->with('success','Data Updated successfully');
