        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="card card-primary p-2 mt-3">
                    ...
                    @if(@$data->form_type == 1)
                        @include('backend.butex_form_builder.custom_excel_form1')  
                    @elseif(@$data->form_type == 2)
                        @include('backend.butex_form_builder.custom_excel_form2')
                    @elseif(@$data->form_type == 3)
                        @include('backend.butex_form_builder.custom_excel_form3')
                    @elseif(@$data->form_type == 4)
                        @include('backend.butex_form_builder.custom_excel_form4')
                    @elseif(@$data->form_type == 5)
                        @include('backend.butex_form_builder.custom_excel_form5')
                    @endif
...
@if(@$data->form_type == 1)
    @php $data = @$data->data1 @endphp
@elseif(@$data->form_type == 2)
    @php $data = @$data->data2 @endphp
@elseif(@$data->form_type == 3)
    @php $data = @$data->data5 @endphp
@elseif(@$data->form_type == 4)
    @php $data = @$data->data2 @endphp
@elseif(@$data->form_type == 5)
    @php $data = @$data->data6 @endphp
@endif
    ...
    <script>
        $('#dataTable2').DataTable({
            ...
            buttons: [
                {
                    extend: 'excel',
                    title: '{{ @$data }}'  // Fixed table name for export
                },
