...
<div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h4 class="m-0 text-dark">{{ (isset($profile_research_gates) && $profile_research_gates->count() > 0)? "Update":"Add" }} Research Gate Info</h4>
        </div>
        <div class="col-sm-6">
  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <h5>
          @if((isset($profile_research_gates) && $profile_research_gates->count() > 0))
          @lang('Update') @lang('Research Gate Info')
          @else
          @lang('Add') @lang('Research Gate Info')
          @endif
      ...
      <div class="tab">
        ...
        @if(@$profile_id)
        <a class="tablinks active" style="color: black;">Research Gate</a>
        @endif
      ...
      <div class="card-body">
        <form method="post" action="{{(@$profile_id)?route('profile_research_gate_info_update',$profile_id):route('profile_research_gate_info_store')}}" id="" enctype="multipart/form-data">
            @csrf
            <div id="research_gate_info" class="tabcontent">
                <h4>Research Gate Info</h4>
                {{-- class="form-row" removed for conflict of minimizing ckeditor features --}}
                <div class="" id="add_education_extra_div">
                  @if(!empty($profile_research_gates))
                  @foreach($profile_research_gates as $key => $profile_research_gate)
                  <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
                      <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
                          <div class="row">
                              <div class="form-group col-sm-12">
                                  ...
                                  <input type="hidden" value="{{ !empty($profile_research_gate->id)? $profile_research_gate->id : "" }}" name="research_gate_id[{{$key}}]">
                                  <textarea id="" name="ResearchGate[{{$key}}]" class="form-control textarea">{{ !empty($profile_research_gate->ResearchGate)? $profile_research_gate->ResearchGate : "" }}</textarea>
                                  @error('ResearchGate')
                      ...
                      <div class="col-1" style="text-align: center;writing-mode: vertical-lr;background: #e6e6b9;border-radius: 0px 10px 10px 0px;">
                          ...
                          <div class="form-group col-md-2">
                              {{-- @if($key != 0) --}}
                              <a id="delete" title="Delete" data-id="{{@$profile_research_gate->id}}" data-token="{{csrf_token()}}" data-route="{{route('remove_profile_research_gate')}}"> <i class="btn btn-danger fas fa-trash"> </i>  </a>
                              {{-- @endif --}}
                  ...
                  @if(empty($profile_research_gates) || count($profile_research_gates) == 0)
                  <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
                      <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
                          <div class="row">
                              <div class="form-group col-sm-12">
                                  <label>Research Gate</label>
                                  <textarea id="" name="ResearchGate[5000]" class="form-control textarea">{{ @$profile_research_gate->ResearchGate }}</textarea>
                                  @error('ResearchGate')
                                  <span class="invalid-feedback" role="alert">
                                      <strong>{{ $message }}</strong>
                                  </span> @enderror
                ...
                <div class="form-row">
                  <div class="form-group col-md-3" style="margin-top: 20px;">
                      <button type="submit" class="btn btn-primary">{{(isset($profile_research_gates) && $profile_research_gates->count() > 0)?"Update":"Save"}}</button>
                  </div>
                </div>
            </div>
    ...
    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
            <div class="row">
                <div class="form-group col-sm-12">
                    <label>Research Gate</label>
                    <textarea id="" name="ResearchGate[@{{counter}}]" class="form-control textarea"></textarea>
                    @error('ResearchGate')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span> @enderror
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on("click",".add_education_extra",function(){
            ...
            $('.select2').select2();
            CKEDITOR.replace('ResearchGate['+counter+']');
            counter ++;
        });
