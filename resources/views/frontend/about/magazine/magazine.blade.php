...
@section('content')
    ...
    <section>
        <div class="container mb-5 mt-5">
            <div class="row rounded" style="height:200px; background-color: #f1f1f1">
                <div class="col-md-4 d-block my-auto justify-content-center align-items-center">
                    <h3 class="text-uppercase fs-4">Magazine SEARCH</h3>
                    <p>Use the filters below to find magazine!</p>
                </div>
    ...
    <main>
        <section id="magazine">
            <div class="academic-details mb-5">
                <div class="container">
                    <div class="row">
                        ...
                        <div class="col-md-8" id="magazine-list">
                            <div class="row">
...
@section('script')
    <script>
        $(document).on('click', '.year', function() {
            var targetDiv = $('#magazine');
            var offset = 160; // Adjust the offset as needed
            var targetPosition = targetDiv.offset().top - offset;
            $(window).scrollTop(targetPosition);

            var year = $(this).data('year');
            $.ajax({
                url: "{{ route('magazine_by_year') }}",
                type: "GET",
                data: {
                    year: year
                },
                success: function(data) {
                    $('#magazine-list').html(data);
                }
            });
        });
    </script>
        ...
        searchInput.addEventListener('input', () => {
            ...
            for (let i = 0; i < dataList.length; i++) {
                ...
                if ((itemText.toLowerCase()).includes((searchTerm.toLowerCase()))) {
                    ...
                    dataList[i].parentNode.parentNode.style.display = 'block';
                } else {
                    dataList[i].parentNode.parentNode.style.display = 'none';
                }
            }
        });
    </script>
