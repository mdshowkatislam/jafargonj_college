...
@section('content')
    <div class="faculty-profile-banner d-flex justify-content-center align-items-center" style=" background-image: url( {{ @$banner->image ? asset('upload/banner/' . $banner->image) : asset('frontend/assets/img/bup/banner.jpg') }} ) ">
        <h1 class="text-white font-poppins">{{ @$type }}</h1>
    </div>
        ...
        <div class="container my-5">
            <div class="row rounded" style="height:11rem; background-color: #f1f1f1">
                ...
                <div class="col-md-8 my-auto justify-content-center">
                    <form action="{{ route('faculty_facultyNewsEventNoticeFilter', $faculty->id) }}" method="post" enctype="multipart/form-data" autocomplete="off" id="myForm">
                        @csrf
                        <div class="input-group" style="height : 60px;">
                            <input type="hidden" name="faculty_id" id="faculty_id" value="{{$faculty->id}}">
                            ...
                            <input type="search" name="search" class="form-control search-box" value="{{ @$query }}" placeholder="Enter Keywords ..."
    ...
    <main class="container">
        <section>
            <div class="container">
                <div class="row">
                    @if (count($news) > 0)
                    {{-- <div class="mt-2 col-12 pb-2">
                        <div class="row d-flex justify-content-center">
                            <div class="col-lg-12 shadow bg-light rounded p-3">
                                <h1 class="fs-4 fw-bold mb-2">
                                    <a href="{{ route('type.details', ["id"=>$n->id,"type"=>'notice','url'=>$url]) }}" class="news-title">{{ $n->title }}
                                    </a>
                                </h1>
                    ...
                    @else
                        <div class="my-5 col-12 ">
                            <div class="p-3 bg-light rounded">
                                <h1 class="fs-5 fw-bold m-0">There are no {{$page_title}} at this moment !</h1>
                            </div>
                        </div>
                    @endif
